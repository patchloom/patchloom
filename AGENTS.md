...
## Using the patchloom CLI

See [PATCHLOOM.md](PATCHLOOM.md) for how to use the patchloom CLI: command reference, exit codes, write modes, transaction plans, and examples. That file is generated by `patchloom agent-rules` and kept in sync by `make sync-patchloom-md`.

When working in this repo, prefer patchloom commands over native agent tools for file operations (read, search, replace, structured edits). See the "Prefer patchloom over raw tools" table in PATCHLOOM.md.
