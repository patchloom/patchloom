...
| `make windows-smoke` | PowerShell dogfood (`scripts/windows-smoke.ps1`: peels, tx, CRLF, rename force/binary path-only, hardlink sibling replace; backslash paths on Windows, nested paths on macOS/Linux pwsh). Not part of `check`; CI `ci-windows`. Requires `pwsh` |
