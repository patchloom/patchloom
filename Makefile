...
dco-check-test: ## Lock DCO sign-off and reject Claude co-author trailers
...
fuzz: ## Run fuzz tests (requires nightly). Use FUZZ_TIME=N for seconds per target.
	...
	for target in fuzz_selector_parse fuzz_patch_parse fuzz_patch_apply fuzz_batch_tokenize fuzz_selector_eval fuzz_doc_parse fuzz_containment_check fuzz_fallback_resolve fuzz_ast_parse fuzz_md_heading fuzz_replace_regex fuzz_shell_token; do \
