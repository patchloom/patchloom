...
- Text I/O honesty (#1894): sole-path loads use `files::load_text_strict` (binary / invalid UTF-8 → `InvalidInput` / `error_kind: invalid_input`); walks use soft-skip `read_text_file` / `read_and_probe`. Byte rule: `classify_text_bytes`.
