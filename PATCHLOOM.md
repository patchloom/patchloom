<!-- Generated by patchloom v0.15.3 — https://github.com/patchloom/patchloom -->
# Patchloom
...
- Text I/O honesty (#1894): sole-path loads use `files::load_text_strict` (binary / invalid UTF-8 → `InvalidInput` / `error_kind: invalid_input`); walks use `try_read_text_file` / `SoftTextSkip` (content soft-skip vs `Unreadable` — do not report pattern `no_matches` when unreadable paths may have masked a scan). Byte rule: `classify_text_bytes`.
...
**Binary paths (#1813):** Sole explicit binary **replace**, **search**, **tidy**, **read**, **md**, or **ast** is `invalid_input` (not pattern `no_matches` / not unsupported-language / not missing-symbol; NUL is valid UTF-8 so text load can succeed without a binary probe). Multi-file **replace**, **search**, and **tidy** lists put binary co-paths in `refused[]` with `reason: binary` (do not treat a partial success as covering every listed path). Multi-file read reports binary paths under `skipped[]` with `error_kind: invalid_input`. Directory walks still soft-skip binaries without failing the whole search.
