...
**Diagnostic envelopes:** `tidy check --json` and `md lint-agents --json` emit a single object `{ok, issue_count, issues, ...}` (not a bare array). Branch on `ok` / `issue_count`. JSONL stays one issue object per line. (#1854)
