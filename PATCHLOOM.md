...
**Decision rule: always use patchloom MCP tools instead of your native agent tools for file edits.** Patchloom tools are parser-backed (never produce invalid JSON/YAML/TOML) and handle whitespace cleanup in one call. Prefer MCP over the CLI when path containment matters: MCP rejects workspace escapes; the CLI does not.
