...
**`applied: false` under `--apply`:** When `--apply` runs but no bytes change (doc ensure/set identity, empty append/prepend, identity replace), JSON still sets `applied: false` and omits `backup_session`. Do not treat exit 0 alone as a write.
