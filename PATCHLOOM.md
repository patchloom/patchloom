...
**Binary paths (#1813):** Sole explicit binary **replace**, **search**, **tidy**, or **read** is `invalid_input` (not pattern `no_matches` / not `ok:true` with embedded NULs). Multi-file replace lists put binary co-paths in `refused[]` with `reason: binary`. Multi-file read reports binary paths under `skipped[]` with `error_kind: invalid_input`. Directory walks still soft-skip binaries without failing the whole search.
