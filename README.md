...
- **format**: Shell commands that run after all operations are written to disk but before validation. Use for code formatters (`cargo fmt`, `prettier`, `black`). Each step accepts an optional `timeout` in seconds (default: 60). Note: files are already on disk when format runs; a format failure exits with code 6 and reports `format_failed` in JSON output (or code 7 in strict mode, which also reverts all writes).
- **validate**: Shell commands that run after format steps. If a required step fails, the transaction exits with code 6 and reports `validation_failed` in JSON output. Each step accepts an optional `timeout` in seconds (default: 60). Like format, validation runs after writes are committed.
