...
| npm publish | **Trusted Publishing (OIDC)** on the package at npmjs.com | cargo-dist builds `*-npm-package.tar.gz`; `publish-npm` uses `id-token: write` + Node 24 / npm >= 11.5.1. Trusted Publisher: GitHub Actions, org `patchloom`, repo `patchloom`, workflow filename `release.yml` only. No long-lived write token |
| Release secrets | add `HOMEBREW_TAP_TOKEN` and `CARGO_REGISTRY_TOKEN` | `HOMEBREW_TAP_TOKEN` must push to both `homebrew-tap` and `scoop-bucket` (classic PAT `public_repo` is enough for public org repos). Prefer OIDC over `NPM_TOKEN` for npm |
