...
5. **Secondary marketplaces** (PulseMCP submit form, mcp.so, mcpservers.org, awesome lists) — re-check only when deliberately listing; not release-blocking. Prefer **Developer Tools** over **File Systems** on curated lists so copy does not imply a generic filesystem MCP.
