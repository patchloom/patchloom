...
| MCP Registry | `server.json` + `.github/workflows/publish-mcp-registry.yml` | Publishes `io.github.patchloom/patchloom` after each GitHub Release (or `workflow_dispatch`). Auth is GitHub OIDC (`id-token: write`); no long-lived registry token. Cargo ownership uses the visible `mcp-name:` line in the crate README. Wait for crates.io + npm markers before publish. Manual re-run: Actions → Publish MCP Registry → version input |
| Smithery | `mcpb/` + `make pack-mcpb` + `.github/workflows/publish-smithery.yml` | Local stdio MCPB (`patchloom/patchloom`). Pack stamps version from Cargo.toml; runtime uses PATH binary or `npx -y patchloom@ver mcp-server`. Publish soft-skips without `SMITHERY_API_KEY` (dashboard key or `smithery auth token`). Manual: `smithery auth login` then `smithery mcp publish target/mcpb/patchloom-<ver>.mcpb -n patchloom/patchloom` |
