...
| Smithery | `mcpb/` + `make pack-mcpb` + `.github/workflows/publish-smithery.yml` | Local stdio MCPB (`patchloom/patchloom`). Pack stamps version from Cargo.toml; runtime uses PATH binary or `npx -y patchloom@ver mcp-server`. Publish soft-skips without `SMITHERY_API_KEY` (from `smithery auth login` → `smithery auth whoami --full`). Manual: `make pack-mcpb && SMITHERY_API_KEY=… bash scripts/publish-smithery.sh` (REST API; avoids CLI 400) |
