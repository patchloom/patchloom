...
| Smithery | `mcpb/` + `make pack-mcpb` + `.github/workflows/publish-smithery.yml` | Local stdio MCPB (`patchloom/patchloom`). Pack stamps version from `$VERSION` (CI) or Cargo.toml; runtime uses PATH binary or `npx -y patchloom@ver mcp-server`. CI installs `@anthropic-ai/mcpb@2.1.2` via registry tarball + SHA-256 check (Scorecard Pinned-Dependencies / alert #49; not bare `npm install -g pkg@ver`). Publish soft-skips without `SMITHERY_API_KEY` (from `smithery auth login` → `smithery auth whoami --full`). Manual: `make pack-mcpb && SMITHERY_API_KEY=… bash scripts/publish-smithery.sh` (REST API; avoids CLI 400). Tests: `make pack-mcpb-test` |
