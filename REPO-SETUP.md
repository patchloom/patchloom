...
| npm publish | **Trusted Publishing (OIDC)** on the package at npmjs.com | cargo-dist builds `*-npm-package.tar.gz`; `publish-npm` uses `id-token: write` + Node 24 (bundled npm, floor-check >= 11.5.1; no unpinned `npm install -g`). Trusted Publisher: GitHub Actions, org `patchloom`, repo `patchloom`, workflow filename `release.yml` only. No long-lived write token. Before publish, the job injects `mcpName: io.github.patchloom/patchloom` into `package.json` for MCP Registry ownership verification |
| MCP Registry | `server.json` + `.github/workflows/publish-mcp-registry.yml` | Publishes `io.github.patchloom/patchloom` via `workflow_call` from `release.yml` after `publish-npm` and `custom-publish-crates` succeed (not on bare `release:published`, which races the host job). Auth is GitHub OIDC (`id-token: write`); no long-lived registry token. Cargo ownership uses the visible `mcp-name:` line in the crate README; npm uses injected `mcpName`. Manual re-run: Actions → Publish MCP Registry → version input |
| Smithery | `mcpb/` + `make pack-mcpb` + `.github/workflows/publish-smithery.yml` | Local stdio MCPB (`patchloom/patchloom`). Pack stamps version from `$VERSION` (CI) or Cargo.toml; runtime uses PATH binary or `npx -y patchloom@ver mcp-server`. CI packs with `zip`+python only (no npm; clears Scorecard npmCommand #49/#50). Optional local official CLI: `PACK_MCPB_USE_CLI=1`. Publish soft-skips without `SMITHERY_API_KEY`. Manual: `make pack-mcpb && SMITHERY_API_KEY=… bash scripts/publish-smithery.sh`. Tests: `make pack-mcpb-test` |
...
4. **Glama** — listing live at [glama.ai/mcp/servers/patchloom/patchloom](https://glama.ai/mcp/servers/patchloom/patchloom). No API publish; sync from GitHub + `glama.json`. If the directory description drifts from `server.json` (common after the first submit), update it in the Glama UI (human session). Canonical blurb: `Agent-safe structured edits: JSON/YAML/TOML/md/AST, dry-run, batch/tx. Not a filesystem MCP.`
5. **Secondary marketplaces** (mcp.so, mcpservers.org, awesome lists) — re-check only when deliberately listing; not release-blocking. Prefer **Developer Tools** over **File Systems** on curated lists so copy does not imply a generic filesystem MCP.
6. **PulseMCP** — no direct GitHub submit for servers. They [ingest the Official MCP Registry](https://www.pulsemcp.com/submit) daily and process listings **weekly**. Keep Registry publish healthy; for listing fixes email `hello@pulsemcp.com` with the GitHub URL and desired short description.
7. **GitHub repo description / topics / homepage** — optional re-check after major messaging changes (`gh api repos/patchloom/patchloom`).
