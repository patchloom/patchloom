...
- **CLI file operations are unrestricted by default.** `create`, `delete`, `read`, `replace`, `patch`, `rename`, and all `tx` operations accept any path the invoking user can access (including `../` escapes from `--cwd`). `--cwd` only sets the default base for relative paths; it is **not** a containment boundary unless you also pass **`--contain`**, which enables the same PathGuard rejection rules as MCP on every CLI write path (engine-backed and binary/case-only rename).
