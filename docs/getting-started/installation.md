...
Tag form is `patchloom-vX.Y.Z` (for example `patchloom-v0.29.0`). <!-- x-release-please-version -->

```powershell
$ver = "0.29.0"   # x-release-please-version (or pin an older release)
$tag = "patchloom-v$ver"
...
```toml
patchloom = { version = "0.29.0", default-features = false, features = ["ast", "files"] } <!-- x-release-please-version -->
```
