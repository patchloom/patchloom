...
- **What it does:** Soft-skips when the file is missing or the selector is not present. The key is not created. Same as plan/MCP/batch `if_exists` on `doc.set`.
- **Use when:** You want to update a key only if it already exists, without a racy `doc has` first.
- **Prefer instead:** Default `doc set` when a missing file should be `not_found` or a missing selector should create the key.
...
- **`--if-exists`:** Soft-skips a missing file or missing selector (does not create the key). Plan/MCP/batch already had this field; the CLI flag is `#2537`.
...
- **What it does:** Applies newline and whitespace normalization to text files. Binary and invalid UTF-8 files are skipped. With no write-policy flags (and without `--respect-editorconfig`), it enables final-newline and trailing-whitespace fixes and unmixes mixed line endings so it matches the issues bare `tidy check` always reports. Uniform CRLF or LF files stay as they are unless `--normalize-eol` is set. Pass explicit flags (or EditorConfig) to narrow the fix set.
...
| SEARCH/REPLACE unique apply | `parse_search_replace` / `apply_search_replace_blocks` / `apply_search_replace_document` / `looks_like_search_replace` (#2220/#2221/#2584). `apply_patch` / CLI `patch apply` / MCP `apply_patch` detect the grammar. Dest is the path above `-------` after SEARCH. Dest-less applies to `apply_patch`'s path (`file_hint`); multi-file / `apply_patch_file` / CLI / MCP still need dest. Default unique (multi-match is `ambiguous`, no write). `replace_all: true` or CLI `--replace-all` updates every exact match. Empty SEARCH is invalid input. Do not flip `ReplaceOptions.unique` on generic `replace_text`. |
