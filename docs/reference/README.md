...
- **Plan-shaped keys:** Optional `key=value` prefixes on positionals are peeled so pasted plan/MCP keys do not become file bytes: file `content=`/`body=`/`path=`, md `heading=`/`content=`/`bullet=`/`row=`, `file.rename` `from=`/`to=`, `ast.rename`/`ast.replace` `old=`/`new=`, doc `selector=`/`key=`/`value=`.
