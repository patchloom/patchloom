...
| Need | API |
|------|-----|
| Fail-closed text replace | `ReplaceOptions.require_change` + `edit_error_kind` / `classify_error`; prefer `ReplaceOptions::for_agent()` for shared agent primary+fallback policy (`unique`, `require_change`, fuzzy @ `AGENT_MIN_FUZZY_SCORE` 0.90, `allow_absent_old: false`; #1965). Approximate recovery stays a one-line override (`allow_absent_old: true`), not a second constructor (#1980). After fuzzy Apply, refuse over-wide spans with `api::fuzzy_span_suspicious(old, matched_text, score)` / `FuzzySpanPolicy` (#1981) |
