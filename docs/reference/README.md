...
- **Failure behavior:** A missing list file is `not_found`. Invalid UTF-8 (including a UTF-16 BOM) and a UTF-16 list without a BOM (NUL bytes that are still valid UTF-8) are `invalid_input`, not `not_found` of the listed names. On Windows, a drive-relative (`C:ok.txt`) or root-relative (`\ok.txt`) line is `invalid_input` of that dest (same peel as `create C:foo`); the list file is not named as missing. When every listed dest is a missing relative path (`nope.txt`), search/replace still report all-missing `not_found` of the `--files-from` list scope, not of each dest.
