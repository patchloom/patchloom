...
- **Failure behavior:** A missing list file is `not_found`. Invalid UTF-8 (including a UTF-16 BOM) and a UTF-16 list without a BOM (NUL bytes that are still valid UTF-8) are `invalid_input`, not `not_found` of the listed names.
