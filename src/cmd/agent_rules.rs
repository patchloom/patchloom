...
resort for typos in non-AST text (prose, comments), not a general rename tool.\n\
             ...
             **Library embedder undo / post-write (Rust hosts, not CLI-only):**\n\
             ...
             - Text I/O honesty (#1894): sole-path loads use `files::load_text_strict` (binary / invalid UTF-8 → \
               `InvalidInput` / `error_kind: invalid_input`); walks use `try_read_text_file` / `SoftTextSkip` \
               (content soft-skip vs `Unreadable` — do not report pattern `no_matches` when unreadable paths \
               may have masked a scan). Byte rule: `classify_text_bytes`.\n\
