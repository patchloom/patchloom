...
Do not put `for_each` inside `operations` and do not pass a bare path array.\n\
             ...
             **Search `--files-without-match` / MCP `files_without_match`:** When every scanned file contains the pattern, the result is `error_kind: no_matches` (CLI exit 3, empty `files`), same as a content miss. That is not a tool crash.\n\n\
