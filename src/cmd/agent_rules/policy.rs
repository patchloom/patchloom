...
Do not put `for_each` inside `operations` and do not pass a bare path array.\n\
             **CLI dest glob vs `--glob` vs plan `path`:** A search/replace/tidy dest like `*.txt` matches files in the current directory only. Use dest `**/*.txt` or `--glob '*.txt'` (with a directory dest) for nested files. Plan/tx/MCP `path` is one file or directory: do not put `*.txt` there. Use replace `\"glob\": \"*.txt\"` or plan-level `for_each.glob`.\n\
