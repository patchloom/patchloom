...
`--files-with-matches` / `--files-without-match`, and one `batch`/`tx` over N full-file dumps. Use `--jsonl` \
...
binary / mode-only dests are listed then apply refuses (no silent skip on mixed patches).\n\n\
             **Library Codex Begin Patch (#2219):** hosts must call `apply_patch` / \
`apply_patch_file` (they detect `*** Begin Patch`). Dest-deny with \
...
refuses before write/backup (#2008). Prefer per-op honesty; rollup fields are worst-case only.\n\n\
             **Library embedder undo / post-write (Rust hosts, not CLI-only):**\n\
             ...
             - On `Err` after write/backup (`format_failed` or fail-restore), `api::backup_session_from_error(&err)` peels the session id without scraping Display (#2127). Fail-restore (`error_kind: rollback` after a successful undo, or `rollback_failed` when restore also failed) is `applied: false`: the persist did not stay on disk (Windows readonly / sharing lock). Do not treat a backup session alone as a landed write.\n\
             ...
             **Missing paths CLI vs MCP (#1793, #2231):** CLI multi-path `replace` soft-skips missing explicit paths under `skipped[]` and still applies the rest. Plan/batch path ops hard-fail missing files with `not_found` and roll back (atomic) unless `if_exists` / `--if-exists` is set, which soft-skips the missing path like CLI. `if_exists` is also honored on plan/MCP `doc.set` and CLI `doc set --if-exists` (missing file or missing selector; the key is not created) and `file.delete` (missing file). Batch lines accept the same `--if-exists` flag on `doc.set` and `file.delete`. Soft zero-match on existing paths still applies matches and lists `refused[]` with overall success. Under `--json`/`--jsonl`, CLI does not also print a human stderr \"No such file\" line for paths already in `skipped[]` (#1797).\n\
             ...
             **UTF-16 `--files-from` (#2340):** A UTF-16 list (BOM or no BOM) is `invalid_input`, not `not_found` of the list file. Save the list as UTF-8.\n\
             **Do not branch on `ok` alone (#1804):** Prefer `applied: true` (CLI write JSON) or MCP/tx `files_changed > 0` with `status: success` for \"edit landed.\" Soft refuse / no match uses `error_kind`/`status: no_matches` (and `applied: false` / `files_changed: 0`). Partial multi-path: read `refused[]` / `skipped[]`. Format failure: `error_kind: format_failed` with `applied: true` means the write already landed on disk (also `write_applied` for one release; prefer `applied`; #1831). Fail-restore after backup (`rollback` / `rollback_failed`) is `applied: false` even when `backup_session` is set.\n\
...
A globset-unparseable `glob`/`exclude` (for example an unclosed `[`) and a `filter` other than `has_symbol(NAME)` are also `invalid_input` (exit 1), not `parse_error` (exit 4). \
Do not put `for_each` inside `operations` and do not pass a bare path array.\n\
             **CLI dest glob vs `--glob` vs plan `path`:** A search/replace/tidy dest like `*.txt` matches files in the current directory only. Dest `sub/*.txt` matches files in that directory only (not cwd, not nested). Use dest `**/*.txt` or `--glob '*.txt'` (with a directory dest) for nested files. Plan/tx/MCP `path` is one file or directory: do not put `*.txt` there. Use replace `\"glob\": \"*.txt\"` or plan-level `for_each.glob`.\n\
             ...
             **Search max_results:** CLI/MCP/tx `search` with `max_results` keeps full `match_count` (and full `file_count`) but may cap the detailed `matches` array **and** the `files` list in `--count` / `--files-with-matches` / `--files-without-match` modes; when capped, JSON sets `truncated: true` (omitted when complete; #1798). Under `--jsonl`, capped content searches append a final `{\"type\":\"summary\",\"match_count\":N,\"match_emitted\":M,\"truncated\":true}` line so agents still see the total. Do not treat `matches.len()` or `files.len()` as total coverage when `truncated` is true.\n\
             **Search `--files-without-match` / MCP `files_without_match`:** When every scanned file contains the pattern, the result is `error_kind: no_matches` (CLI exit 3, empty `files`). The error text is `no files without matches for 'PATTERN' in SCOPE` so agents do not retry as if the pattern missed. That is not a tool crash and must not tip `-i`.\n\n\
