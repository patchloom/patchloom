...
fn check_render_issues_json_ok() {
    ...
    render_issues(&issues, &global, None, None).expect("json emit must succeed for TidyIssue");
}
...
fn check_render_issues_jsonl_ok() {
    ...
    render_issues(&issues, &global, None, None).expect("jsonl emit must succeed for TidyIssue");
}
