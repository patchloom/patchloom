...
//! ```toml
//! [dependencies]
//! patchloom = { version = "0.37.0", default-features = false } // x-release-please-version
//! ```
//!
//! Or with AST support:
//!
//! ```toml
//! patchloom = { version = "0.37.0", default-features = false, features = ["ast"] } // x-release-please-version
//! ```
//!
...
//! | SEARCH/REPLACE unique apply | [`api::parse_search_replace`] / [`api::apply_search_replace_blocks`] / [`api::apply_search_replace_document`] / [`api::looks_like_search_replace`] (#2220/#2221/#2584). `apply_patch` / `apply_patch_file` / CLI `patch apply` / MCP `apply_patch` detect the grammar. Dest is the path above `-------` after SEARCH. Dest-less applies to `apply_patch`'s path (`file_hint`). Default unique (multi-match is `ambiguous`, no write). `replace_all: true` or CLI `--replace-all` updates every exact match. Do not flip [`ReplaceOptions::unique`] on generic `replace_text`. |
//! | Hunked `+++ /dev/null` delete | Empty-hunk git `deleted file mode` unlinks. A hunked delete applies minus lines first; leftover bytes rewrite the file (preview `--diff` or `EditResult.new_content`). Stale minus is `ambiguous` and does not remove the file. Missing dest peels `not_found` on [`api::apply_patch`] and [`api::apply_patch_file`] (Apply and Check). A directory dest peels `invalid_input`. FIFO, dangling symlink, and binary regular files stay path-only unlinkable (same as [`api::file_delete`]). PathGuard on an escaped missing dest peels `guard_rejected` first. |
...
#[cfg(feature = "files")]
pub use api::expand_for_each;
#[cfg(any(feature = "cli", feature = "files"))]
pub use api::search_one_file;
pub use api::{
    AGENT_MIN_FUZZY_SCORE, ApplyFragmentSpec, ApplyMode, ApplySearchReplaceOptions, CharsetMode,
    ContentEdit, ContentEditHonesty, ContentEditResult, ContentEditsResult, DesugaredReplace,
    EditError, EditErrorKind, EditResult, FragmentPlacement, FuzzySpanPolicy, Hunk, MatchMode,
    PatchFile, PatchLine, PeeledError, PostWriteHooks, PostWriteOnFailure, ReplaceOptions,
    SearchOptions, SearchReplaceBlock, SearchResult, WritePolicyOptions, apply_begin_patch,
    apply_content_edits, apply_content_edits_with_label, apply_post_write_validator,
    apply_search_replace_blocks, apply_search_replace_document, begin_patch_declared_paths,
    build_apply_fragment_spec, build_context_lines, classify_error, classify_error_ref,
    desugar_to_replace_fields, desugar_to_replace_operation, edit_error_kind, edit_error_ref,
    error_kind_str, format_search_results, fuzzy_span_suspicious,
    ...
    lifecycle_cmds, load_text, load_text_strict, looks_like_begin_patch, looks_like_search_replace,
    merge_match_modes, parse_diff_fenced, parse_diff_file_path, parse_diff_git_paths,
    parse_search_replace, parse_search_replace_document, parse_unified_diff, patch_declared_paths,
    ...
    search_file, search_replace_declared_paths, strip_lazy_markers, text_diff,
