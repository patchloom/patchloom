...
//! | Hunked `+++ /dev/null` delete | Empty-hunk git `deleted file mode` unlinks. A hunked delete applies minus lines first; leftover bytes rewrite the file (preview `--diff` or `EditResult.new_content`). Stale minus is `ambiguous` and does not remove the file. Missing dest peels `not_found` on [`api::apply_patch`] and [`api::apply_patch_file`] (Apply and Check). Path-only unlink is [`api::file_delete`]. |
