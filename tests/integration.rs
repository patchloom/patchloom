...
mod tx_tests {
    ...
    fn test_tx_rollback_failed_when_restore_incomplete() {
        ...
        let (code, json_str) =
            patchloom::cmd::tx::execute_plan_direct(plan, dir.path(), None).unwrap();
    ...
    fn test_tx_json_output_on_replace_missing_mode_parse_error() {
        ...
        assert!(
            error.contains("replace operation requires one of to, insert_before, or insert_after")
        );
    }
...
mod batch_tests {
    ...
    fn test_batch_reference_operation_count_matches_parser() {
        ...
        let actual =
            collect_batch_operation_names(&repo_root().join("src").join("cmd").join("batch.rs"))
                .len();
...
mod read_tests {
    ...
    async fn test_mcp_symlink_outside_cwd_rejected() {
        ...
        let params = rmcp::model::CallToolRequestParams::new("read_file").with_arguments(
            serde_json::from_value(serde_json::json!({"path": "escape.txt"})).unwrap(),
        );
        let result = client.peer().call_tool(params).await;
...
mod explain_tests {
    ...
    fn test_explain_invalid_plan_fails() {
        ...
        Command::cargo_bin("patchloom")
            ...
            .failure()
            .stderr(
                predicate::str::contains("expected ident").or(predicate::str::contains("parse")),
            );
    }
...
mod init_tests {
    ...
    fn test_init_falls_back_to_completion_command_when_completion_dir_creation_fails() {
        ...
        assert!(
            stderr
                .contains("patchloom completions fish > ~/.config/fish/completions/patchloom.fish")
        );
    }
...
mod schema_tests {
    ...
    fn test_schema_tier_filtering() {
        ...
        let weak_json: serde_json::Value = serde_json::from_str(
            &String::from_utf8(weak_output.get_output().stdout.clone()).unwrap(),
        )
        .unwrap();
    ...
    fn test_schema_examples_flag() {
        ...
        let with_json: serde_json::Value = serde_json::from_str(
            &String::from_utf8(with_output.get_output().stdout.clone()).unwrap(),
        )
        .unwrap();
        let with = with_json
            .get("operations")
...
mod mcp_tests {
    ...
    fn test_mcp_setup_documents_search_files_modes() {
        let doc =
            fs::read_to_string(repo_root().join("docs/getting-started/mcp-setup.md")).unwrap();
    ...
    async fn test_mcp_https_search_files_round_trip() {
        ...
        let ca_cert =
            reqwest::tls::Certificate::from_pem(cert_pem.as_bytes()).expect("parse CA cert");
        ...
        let params = rmcp::model::CallToolRequestParams::new("search_files".to_string())
            .with_arguments(
                serde_json::from_value(serde_json::json!({"pattern": "tls", "paths": ["."]}))
                    .unwrap(),
            );
        let result = client.peer().call_tool(params).await.unwrap();
        assert!(
...
mod smoke_tests {
    ...
    fn test_smoke_quickstart_command_flow() {
        ...
        assert!(
            quickstart
                .contains("patchloom replace 'old_function' --to 'new_function' src/ --apply")
        );
...
mod cli_flags_tests {
    ...
    async fn test_mcp_concurrent_doc_set() {
        ...
        let params_a = rmcp::model::CallToolRequestParams::new("doc_set".to_string())
            .with_arguments(
                serde_json::from_value(
        ...
        let params_b = rmcp::model::CallToolRequestParams::new("doc_set".to_string())
            .with_arguments(
                serde_json::from_value(
        ...
        let params_c = rmcp::model::CallToolRequestParams::new("doc_set".to_string())
            .with_arguments(
                serde_json::from_value(
...
mod reference_docs_tests {
    ...
    fn test_reference_doc_describes_status_and_undo_contracts() {
        ...
        assert!(
            reference
                .contains("This command is git-backed, so it must run inside a git repository.")
        );
    ...
    fn test_reference_doc_describes_create_input_contract() {
        ...
        assert!(
            reference.contains(
                "Passing both is rejected with `--content and --stdin cannot be combined`"
            )
        );
        assert!(reference.contains(
...
mod misc_tests {
    ...
    fn test_workflows_disable_persisted_checkout_credentials_by_default() {
        ...
        let security =
            fs::read_to_string(repo_root().join(".github/workflows/security.yml")).unwrap();
    ...
    fn test_agent_driver_subcommands_match_cli() {
        ...
        let mut cli_commands: std::collections::BTreeSet<String> =
