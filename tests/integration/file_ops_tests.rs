...
/// #2361: drive-relative dests in `--files-from` peel `invalid_input`
/// of the dest, not `not_found` of the list file.
#[cfg(windows)]
#[test]
fn test_files_from_drive_relative_line_is_invalid_input() {
    let dir = TempDir::new().unwrap();
    fs::write(dir.path().join("ok.txt"), "hit\n").unwrap();
    fs::write(dir.path().join("list.txt"), "C:ok.txt\n").unwrap();
    ...
    assert_eq!(
        parsed["error_kind"], "invalid_input",
        "drive-relative list dest must not peel not_found of the list: {parsed}"
    );
    let err = parsed["error"].as_str().unwrap_or("");
    assert!(
        err.contains("drive-relative") || err.contains("ignores --cwd"),
        "error should name the dest class: {err}"
    );
    assert!(
        err.contains("C:ok.txt"),
...
#[cfg(windows)]
#[test]
fn test_files_from_root_relative_line_is_invalid_input() {
    let dir = TempDir::new().unwrap();
    fs::write(dir.path().join("ok.txt"), "hit\n").unwrap();
    ...
    assert_eq!(
        parsed["error_kind"], "invalid_input",
        "root-relative list dest must not peel not_found of the list: {parsed}"
    );
    let err = parsed["error"].as_str().unwrap_or("");
    assert!(
        err.contains("drive-relative") || err.contains("ignores --cwd"),
