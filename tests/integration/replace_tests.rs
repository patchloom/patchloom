...
/// Sole unreadable is invalid_input (not pattern no_matches).
#[test]
fn test_replace_sole_unreadable_is_invalid_input() {
    let dir = TempDir::new().unwrap();
    ...
    #[cfg(unix)]
    {
        ...
        let out = Command::cargo_bin("patchloom")
            ...
            .args(["--json", "--cwd"])
            .arg(dir.path())
            .args(["replace", "hello", "--new", "hi", "locked.txt", "--apply"])
            .output()
            .unwrap();
...
#[test]
fn test_replace_dir_all_unreadable_not_no_matches() {
    let dir = TempDir::new().unwrap();
    let a = dir.path().join("a.txt");
    fs::write(&a, "hello\n").unwrap();
    #[cfg(unix)]
    {
        ...
        let out = Command::cargo_bin("patchloom")
            ...
            .args(["--json", "--cwd"])
            .arg(dir.path())
            .args(["replace", "hello", "--new", "hi", ".", "--apply"])
            .output()
            .unwrap();
